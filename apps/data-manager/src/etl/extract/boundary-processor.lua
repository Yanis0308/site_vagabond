...
function M.process_boundary(tables, object, geom)
  ...
  if not admin_level_int then
    print(
      "Warning: Invalid admin_level '" .. tostring(
        object.tags.admin_level
      ) .. "' for boundary " .. tostring(object.id) .. " - skipping"
    )
  ...
  if object.members then
    for _, member in ipairs(object.members) do
      if member.role == "admin_centre" then
        ...
        print(
          "Found admin_centre for boundary " .. tostring(
            object.id
          ) .. ": " .. member.type .. " " .. tostring(member.ref)
        )
      end
    end
  end
  ...
  if #admin_centre_members > 0 then
    ...
    for _, member in ipairs(admin_centre_members) do
      table.insert(
        json_parts,
        '{"type":"' .. member.type .. '","ref":' .. tostring(member.ref) .. "}"
      )
    end
    admin_centre_members_json = "[" .. table.concat(json_parts, ",") .. "]"
    print(
      "Stored admin_centre_members for boundary " .. tostring(
        object.id
      ) .. ": " .. admin_centre_members_json
    )
  end
