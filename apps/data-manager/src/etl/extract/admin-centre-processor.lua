...
function M.register_admin_centre_ref(
boundary_osm_id,
  boundary_osm_type,
  admin_centre_ref,
  admin_centre_type
...
function M.process_admin_centre(tables, object, geom)
  ...
  if not display_name or display_name == "" then
    print(
      "Warning: admin_centre node " .. tostring(
        object.id
      ) .. " has no name or label - skipping"
    )
    return
  end
  ...
  if place_type and not is_valid_place_type(place_type) then
    print(
      "Warning: admin_centre node " .. tostring(
        object.id
      ) .. " has invalid place type '" .. tostring(
        place_type
      ) .. "' - will be null"
    )
    place_type = nil
  ...
  print(
    "Processing admin_centre: " .. display_name .. " (place=" .. tostring(
      place_type
    ) .. ", pop=" .. tostring(population) .. ", score=" .. tostring(
      current_admin_centre.importance_score
    ) .. ")"
  )
