...
for name, definition in pairs(table_definitions) do
  ...
  for _, column in ipairs(table_columns[name]) do
    if type(column) == "table" then
      ...
      table.insert(definition.columns, {
        column = column,
        type = "text",
      })
...
local ignore_key_prefixes = {
  "note:",
  "source:", -- Corine (CLC) (Europe)
  "CLC:", -- Geobase (CA)
  "geobase:", -- CanVec (CA)
  "canvec:", -- Geobase (CA)
  "geobase:", -- Friuli Venezia Giulia (IT)
  "it:fvg:", -- KSJ2 (JA)
  ...
  "NHD:",
  "nhd:", -- mvdgis (Montevideo, UY)
  "mvdgis:", -- Land Information New Zealand
  "ref:linz:", -- Los Angeles County
  "lacounty:",
}
...
local function prepare_columns(tags, tag_map, ignore_type)
  local attrs = {
    tags = {},
    layer = normalize_layer(tags.layer),
  }
...
local function process_relation(object)
  ...
  if type == "boundary" or (type == "multipolygon" and object.tags.boundary) or type == "route" then
    local geom =
      object:as_multilinestring():line_merge():transform(3857):segmentize(
        100000
      )
    add_linear("line", attrs, geom)
