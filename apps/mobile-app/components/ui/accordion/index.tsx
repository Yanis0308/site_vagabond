...
type IAccordionTriggerProps = Omit<
...
// Helper component to extract items from FlashList and render them statically
const FlashListAccordionWrapper = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const items = useMemo(() => {
    // Extract items from FlashList by checking if it's a FlashList
    ...
    for (const child of childArray) {
      if (React.isValidElement(child)) {
        ...
        const isFlashList =
          childType?.displayName === "FlashList" ||
          childType?.name === "FlashList" ||
          (typeof childType === "function" &&
            childType.toString().includes("FlashList"));
        ...
        if (isFlashList) {
          ...
          if (props.data && props.renderItem) {
            // Pre-render all items from FlashList data
...
>(({ className, variant = "filled", size = "md", children, ...props }, ref) => {
  // Check if children contains FlashList
  const hasFlashList = useMemo(() => {
  ...
  // If FlashList is detected, use wrapper to extract items statically
  const processedChildren = useMemo(() => {
    if (hasFlashList) {
      return <FlashListAccordionWrapper>{children}</FlashListAccordionWrapper>;
    }
    return children;
  }, [children, hasFlashList]);
