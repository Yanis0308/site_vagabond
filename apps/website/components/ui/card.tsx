...
function Card({
  ...
  size = "default",
  ...props
}: ComponentProps<"div"> & { size?: "default" | "sm" }): ReactNode {
  return (
    <div
      data-slot="card"
      data-size={size}
      className={cn(
        `
          group/card flex flex-col gap-6 overflow-hidden rounded-2xl bg-card py-6 text-sm text-card-foreground ring-1
          ring-foreground/10
